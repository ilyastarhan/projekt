package Homework.eylul22;

import java.util.Scanner;

public class Task_SeyahatProjesi {

    public static void main(String[] args) {

        /*
        bulundugunuz yerden, bir yere yolculuk edeceksiniz.

        1. Köln veya Frankfurta gidebilirsiniz.
        2. Bilet fiyatı hesabı = Her 20 Km başına 5 euro. örn: (100 km yol. Bilet parası  (100/20)* 5 =25 euro

todo İPUCU:  toplamTutar, işlem, FrankfurtKm, KmBirimFiyat   gibi sabit veri tipleri oluşturabilirsiniz.
 İşlemlerde direkt bunları cağırabilirsiniz.

        todo İLK OLARAK;

        "Nereye yolculuk etmek istiyorsunuz? (Frankfurt : 60 KM          ---         Köln : 80 KM ---       (20 KM başına 5 euro bilet parası alınmaktadir..)    yazısı gelsin.

    1.    Frankfurt veya Köln olarak bir giriş yapın. (Girdiğiniz sehrin harfleri , girildikten sonra büyük hale gelsin.

   todo    Frankfurt girildiyse,
        örn: girdi= Frankfurt
                case=FRANKFURT:

      Rota = Frankfurt yazdırın.
      Frankfurt km hesabı işlemi yapın.

      son olarak konsolda: Frankfurt 15 Euro         yazsın.
      todo  case: KÖLN ise

      "Rota = Köln" yazdırın.
       km hesabına göre işlemi yapınız.

       son olarak konsolda:  Köln   20 Euro             yazsın.



       2.  Sistem bize "Kaç kişilik bilet istiyorsunuz? (Max 2 kişilik olabilir):   "   sorusunu sorsun.

       case:1  ise
        " 1 kişilik " yazdırın. (1 kişilik seçerseniz bir işlem yapmanıza gerek kalmaz, bilet ücreti yukarıda çoktan belirlenmiş olur.)

        case:2 ise

        "2 kişilik " yazdırın
        2 kişi için bilet fiyatını hesaplayın.


        todo  Son olarak yolculugunuzu check edin;

        konsolda örn:   Frankfurt  - 2 Kişilik    yazsın.

        1.Bakiyemi belirtin.
        2.Toplam Tutarı belirtin.
        3.double paraUstu oluşturun ve hesaplayın.
        4.Double para üstünü yazdırın.


         */

        Scanner scan = new Scanner(System.in);
        double bilet;
        int frankfurtkm = 80;
        int kolnkm = 60;
        double kmfiyat = 0.25;

        System.out.println("***** seyahat şirketimize hos geldiniz *****");
        System.out.println("1- Frankfurt : 60 km \n2- Koln : 80 km\n 20 Km basına 5 euro ucretlendirme yapılır :) ");
        System.out.print("yolculuk yapacagınız sehri yaziniz : ");

        String hedef = scan.next().toUpperCase();//sehir buyuk harfe update edildi
        switch (hedef) {
            case "FRANKFURT":
                System.out.println("Rota = Frakfurt ");
                bilet = frankfurtkm * kmfiyat;
                System.out.println("Tek kisilik Frankfurt bileti " + bilet + " Euro'dur");
                System.out.println("Kac kisilik bilet almak istiyorsunuz? (Max 2 kisilik olabilir): ");
                int kisi = scan.nextInt();
                switch (kisi) {
                    case 1:
                        System.out.println("1 kisilik Frankfurt bilet ücreti: " + bilet + " Euro'dur");
                        break;
                    case 2:
                        bilet *= 2;
                        System.out.println("2 kisilik Frankfurt bilet ücreti: " + bilet + " Euro'dur");
                        break;
                    default:
                        System.out.println("Hatali bir sayi girdiniz, lütfen daha sonra tekrar deneyiniz...");
                }
                System.out.println("Koln" + kisi + "Kisilik");
                System.out.println("Lütfen ödeme tutarini giriniz: ");
                double tutar = scan.nextDouble();
                double paraustu = (tutar - bilet);
                System.out.println("paraustu = " + paraustu);
                break;
            case "KOLN":
                bilet = kolnkm*kmfiyat;
                System.out.println("Rota = Köln ");
                System.out.println("Tek kisilik Koln bileti " + bilet + " Euro'dur");
                System.out.println("Kac kisilik bilet almak istiyorsunuz? (Max 2 kisilik olabilir): ");
                kisi = scan.nextInt();
                switch (kisi) {
                    case 1:
                        System.out.println("1 kisilik Koln bilet ücreti: " + bilet + " Euro'dur");
                        break;
                    case 2:
                        bilet *= 2;
                        System.out.println("2 kisilik Koln bilet ücreti: " + bilet + " Euro'dur");
                        break;
                    default:
                        System.out.println("Hatali bir sayi girdiniz, lütfen daha sonra tekrar deneyiniz...");
                                        }
                System.out.println("Koln" + kisi + "Kisilik");
                System.out.println("Lütfen ödeme tutarini giriniz: ");
                tutar = scan.nextDouble();
               paraustu = (tutar - bilet);
                System.out.println("paraustu = " + paraustu);
                break;
            default:
                System.out.println("Üzgünüz...\nSu an icin sadece Frankfurt ve Köln sehirlerine yerimiz vardir...");



        }



    }
}
